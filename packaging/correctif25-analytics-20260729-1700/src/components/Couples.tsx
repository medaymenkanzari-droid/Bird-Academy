...
export default function Couples({
  ...
  const activeCoupledBirdIds = couples
    .filter(c => c.statut === 'Actif')
    .flatMap(c => [c.male_id, c.femelle_id]);

  const availableMales = canaris.filter(bird => 
    bird.sexe === 'Mâle' && !activeCoupledBirdIds.includes(bird.id)
  );

  const availableFemales = canaris.filter(bird => 
    bird.sexe === 'Femelle' && !activeCoupledBirdIds.includes(bird.id)
  );

  // Consanguinity calculation variables
  const showCOI = isFormOpen && selectedMaleId && selectedFemelleId;
  const maleIdNum = Number(selectedMaleId);
  const femaleIdNum = Number(selectedFemelleId);
  const coi = showCOI ? calculateInbreedingCOI(maleIdNum, femaleIdNum, canaris) : 0;
  const coiCategory = getInbreedingCategory(coi);
  const relationship = showCOI ? getRelationshipLabel(maleIdNum, femaleIdNum, canaris, t) : '';
  ...
  // Select a couple to inspect details/history
  const selectedCouple = couples.find(c => c.id === selectedCoupleId);
  const selectedMale = selectedCouple ? canaris.find(b => b.id === selectedCouple.male_id) : null;
  const selectedFemelle = selectedCouple ? canaris.find(b => b.id === selectedCouple.femelle_id) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">{labels.couplesTitle}</h2>
          <p className="text-xs text-slate-500">{labels.couplesSub}</p>
        </div>
        <button
          onClick={() => {
            setErrorMsg(null);
          ...
          className="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white font-semibold px-4 py-2 rounded-xl transition-colors text-sm cursor-pointer shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2 columns: Active Couples List */}
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white p-4 rounded-xl border border-slate-100">
            <h3 className="text-sm font-semibold text-slate-700 mb-3">{labels.couplesListTitle}</h3>
                ...
                {couples.map((c) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl border transition-all cursor-pointer flex flex-col sm:flex-row items-center justify-between gap-4 ${
                        isSelected 
                          ? 'bg-rose-50 border-rose-300 shadow-xs' 
                          : 'bg-white border-slate-100 hover:border-slate-200 shadow-xs'
                      }`}
                    >
                      {/* Couple Birds Representation */}
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        {/* Male Card Mini */}
                        <div className="bg-blue-50 border border-blue-100 rounded-lg p-2 flex items-center gap-2 shrink-0">
                          <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded">{labels.maleLabel}</span>
                          <div className="text-xs">
                            <div className="flex items-center gap-1.5"><div className="font-bold text-slate-800 leading-tight">{male ? male.nom : labels.unknownMale}</div>{male && <SpeciesBadge speciesId={male.espece} size="sm" showLabel={false} />}</div>
                            <div className="font-mono text-[9px] text-slate-400">{male ? male.bague : "—"}</div>
                          </div>
                        </div>

                        {/* Heart Link icon */}
                        <Heart className={`w-5 h-5 shrink-0 ${c.statut === 'Actif' ? 'text-rose-500 fill-rose-50' : 'text-slate-300'}`} />

                        {/* Female Card Mini */}
                        <div className="bg-rose-50 border border-rose-100 rounded-lg p-2 flex items-center gap-2 shrink-0">
                          <span className="text-[10px] font-bold text-rose-600 bg-rose-100 px-1.5 py-0.5 rounded">{labels.femaleLabel}</span>
                          <div className="text-xs">
                            <div className="flex items-center gap-1.5"><div className="font-bold text-slate-800 leading-tight">{femelle ? femelle.nom : labels.unknownFemale}</div>{femelle && <SpeciesBadge speciesId={femelle.espece} size="sm" showLabel={false} />}</div>
                            <div className="font-mono text-[9px] text-slate-400">{femelle ? femelle.bague : "—"}</div>
                          </div>
                        </div>
                      </div>

                      {/* Right Meta Info & Quick Actions */}
                      <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4 pt-3 sm:pt-0 border-t sm:border-0 border-slate-50">
                        <div className="text-left sm:text-right text-xs">
                          <span className={`inline-block px-2 py-0.5 rounded-full font-bold text-[10px] ${
                            c.statut === 'Actif' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                          }`}>
                            {c.statut === 'Actif' ? t('activeCouples').slice(0, -1) || 'Actif' : c.statut}
                          </span>
          ...
          {isFormOpen ? (
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-800 text-md">{labels.formCoupleTitle}</h3>
                </div>
                <button
                  type="button"
              ...
              {/* Select Male */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">{labels.selectMaleLabel}</label>
                <select
                  required
                  value={selectedMaleId}
                  onChange={(e) => setSelectedMaleId(e.target.value)}
                  className="w-full p-2.5 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-1 focus:ring-rose-500"
                >
              ...
              {/* Select Female */}
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">{labels.selectFemaleLabel}</label>
                <select
                  required
              ...
              {selectedMaleId && selectedFemelleId && (
                <div className={`p-4 rounded-xl border ${coiCategory.bgClass} ${coiCategory.borderClass} space-y-3`}>
                  ...
                  <div className="pt-2 border-t border-slate-200/60">
                    ...
                    {showScaleInfo && (
                      <div className="mt-2 bg-white/80 p-2.5 rounded-lg border border-slate-100 text-[10px] space-y-1.5">
                        <div className="grid grid-cols-3 font-bold text-slate-600 border-b border-slate-100 pb-1">
                        ...
                        <div className="grid grid-cols-3 text-emerald-700 font-medium">
                          <span>0 %</span>
                          <span>{t('riskNone')}</span>
                          <span>{t('recRecommended')}</span>
                        </div>
                        <div className="grid grid-cols-3 text-green-700 font-medium">
                          <span>&gt; 0 - 6,25 %</span>
                          <span>{t('riskLow')}</span>
                          <span>{t('recAcceptable')}</span>
                        </div>
                        <div className="grid grid-cols-3 text-amber-700 font-medium">
                          <span>6,25 - 12,5 %</span>
                          <span>{t('riskMedium')}</span>
                          <span>{t('recPrudence')}</span>
                        </div>
                        <div className="grid grid-cols-3 text-orange-700 font-medium">
                          <span>12,5 - 25 %</span>
                          <span>{t('riskHigh')}</span>
                          <span>{t('recNotRecommended')}</span>
                        </div>
                        <div className="grid grid-cols-3 text-red-700 font-bold">
                          <span>&gt; 25 %</span>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
              <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-sm">{labels.coupleCardTitle.replace('{id}', selectedCouple.id.toString())}</h3>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  selectedCouple.statut === 'Actif' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                }`}>
                  {selectedCouple.statut === 'Actif' ? t('activeCouples').slice(0, -1) || 'Actif' : selectedCouple.statut}
                </span>
              </div>
              ...
              {selectedCouple.statut === 'Actif' && (
                <div className="p-3 bg-rose-50/50 rounded-xl border border-rose-100 text-xs">
                  ...
                  <p className="text-slate-600 mt-1">{labels.cycleReadyDesc}</p>
                ...
                {coupleReproductions.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                    {coupleReproductions.map((r) => (
                      <div key={r.id} className="p-2.5 bg-slate-50 border border-slate-100 rounded-lg text-xs flex justify-between items-center">
                        <div>
                          <span className="font-bold text-slate-700">Reproduction #{r.id}</span>
