...
export const LineageAnalysis: React.FC<LineageAnalysisProps> = ({ birds }) => {
  ...
  const individualWrightResult = useMemo(() => {
    ...
    // No parents means F=0
    return {
      coefficient: 0,
      level: 'none' as const,
      commonAncestors: [],
      explanation: "L'oiseau n'a pas de parents déclarés. Coefficient de consanguinité nul par défaut.",
      pedigreeDepth: 0
    };
  }, [selectedBird, birds]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl flex flex-col h-[600px] shadow-2xs">
          <div className="flex items-center gap-1.5 pb-3 border-b border-slate-100 dark:border-slate-800 mb-4">
            ...
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-200">Sélectionner un Sujet</h3>
          </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          {selectedBird && lineageAnalysis && individualWrightResult ? (
              ...
              <div className="p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl flex items-center justify-between shadow-2xs">
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">
                    Fiche Génétique & Lignée
                  </h3>
                  <p className="text-xs text-slate-500 font-semibold mt-1">
                    Analyse de pedigree pour : <span className="text-indigo-600 dark:text-indigo-400">{selectedBird.nom || 'Sujet sans nom'}</span> ({selectedBird.bague})
                  </p>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-bold text-slate-400 block">Indice de Diversité</span>
                  <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{lineageAnalysis.diversityIndex}%</span>
                </div>
              </div>
              ...
              <div className="p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xs space-y-4">
                <div className="flex items-center gap-1.5 border-b border-slate-50 dark:border-slate-800 pb-2">
                  ...
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-200">
                    Analyse des Lignées & Branches actives
                  </h4>
                </div>
                ...
                <div className="space-y-4 text-xs">
                  ...
                  <div className="space-y-1.5">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Branches Fondatrices Actives :</span>
                    <div className="flex flex-wrap gap-2">
                      ...
                      {lineageAnalysis.mainBranches.length === 0 && (
                        <span className="text-slate-400 italic">Aucune branche connue.</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Branches à goulot d'étranglement ou fragiles :</span>
                    <div className="flex flex-wrap gap-2">
                      {lineageAnalysis.lostBranches.map((br, idx) => (
                        <span key={idx} className="px-2.5 py-1 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-100/40 dark:border-amber-900/40 font-semibold text-[10px] flex items-center gap-1">
                          ...
                          <span>Branche de {br}</span>
                        </span>
                      ))}
                      {lineageAnalysis.lostBranches.length === 0 && (
                        <span className="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                          ✓ Toutes les branches collatérales ont de multiples descendants actifs (aucun goulot d'étranglement).
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-12 text-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl flex flex-col items-center justify-center gap-2 h-72">
              ...
              <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 font-black">Analyseur de Lignée</h4>
              <p className="text-xs text-slate-400 max-w-sm leading-relaxed">
                Veuillez sélectionner un oiseau dans la colonne de gauche pour dresser le bilan complet de sa lignée généalogique.
              </p>
            </div>
          )}
        </div>
