...
export const AnalyticsHeatMap: React.FC<{
  ...
  return (
    <div className="space-y-2 text-xs">
      <div className="grid grid-cols-6 gap-1 font-mono text-[9px] text-slate-400">
        <span />
        {columnLabels.map(label => <span key={label}>{label}</span>)}
      </div>
      {gridData.map((row, idx) => (
        <div key={idx} className="flex items-center gap-2">
          <span className="w-16 font-semibold text-[10px] text-slate-500 truncate">{row.label}</span>
