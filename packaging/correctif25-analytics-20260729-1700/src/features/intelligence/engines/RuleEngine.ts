  ...
  private static rules: Rule[] = [
    {
      ...
      condition: (data: { pairs: BreedingPair[]; clutches: Clutch[] }) => {
    ...
    },
    {
      ...
      condition: (data: { cages: HabitatCage[]; birds: Canari[] }) => {
        return data.cages.some(cage => {
          const birdCount = data.birds.filter(b => String(b.cage_id) === cage.id || b.cageId === cage.id).length;
          return birdCount > cage.capacite_max;
        });
      }
    },
    {
    ...
    },
    {
      ...
      condition: (data: { birds: Canari[]; pairs: BreedingPair[] }) => {
        ...
        return data.pairs.some(pair => {
          ...
          return wright.coefficient > params.maximumRecommendedCoefficient;
        });
      }
    },
    {
    ...
    },
    {
      ...
      condition: (data: { birds: Canari[] }) => {
        const activeBirds = data.birds.filter(b => !b.archived);
