...
export default function IncubationCalendarView({ clutchId, onRefreshClutches }: IncubationCalendarViewProps) {
  ...
  const milestones = [
    { key: 'laying', label: 'Ponte & Incubation', date: calendarData?.layingDate },
      ...
      <div className="flex justify-between items-center pb-2 border-b border-slate-100">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-bold text-slate-700">
            {t('durationRemaining')} : {calendarData?.durationRemaining} {t('daysPlural')}
          </span>
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-slate-100">
        ...
        {incubation.events.length === 0 ? (
          ...
          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 space-y-2 max-h-[160px] overflow-y-auto">
            {incubation.events.map((e: IncubationEvent) => (
