...
import { 
  ...
  TrendingDown, TrendingUp, BarChart3, Settings, Wheat, Menu, X, Bell, BookOpen, BrainCircuit, Dna, ShieldCheck, ShieldAlert, Database
} from 'lucide-react';
...
// Type declarations
import { 
  Canari, Cage, Couple, Reproduction, Ponte, Jeune, Sante, Alimentation, Depense, Vente 
} from './types';
...
import { AppPage, AppSkeleton, BrandLogoIcon } from './components/design-system';
import { ChunkLoadErrorBoundary } from './components/ChunkLoadErrorBoundary';

const Dashboard = lazy(() => import('./components/Dashboard'));
const Canaris = lazy(() => import('./components/Canaris'));
const Couples = lazy(() => import('./features/reproduction/components/ReproductionComponent'));
...
const PlatformDashboard = lazy(() => import('./features/platform/components/PlatformDashboard').then(module => ({ default: module.PlatformDashboard })));
const QADashboard = lazy(() => import('./features/quality/components/QADashboard').then(module => ({ default: module.QADashboard })));
...
export default function App() {
  ...
  // Check onboarding on initial boot
  useEffect(() => {
  ...
  // 1. Load database from Repositories on initialization
  useEffect(() => {
    const cachedCanaris = BirdRepository.getAll();
    const cachedCages = HabitatRepository.getAll();

    if (cachedCanaris.length > 0 || cachedCages.length > 0) {
      ...
      setAlimentation(HandFeedingService.getOrInitializeDefaultPlans(INITIAL_ALIMENTATION));
      setDepenses(FinanceService.getExpenses());
      setVentes(FinanceService.getSales());
    } else {
      // Initialize with beautiful pre-populated default data on first-time load
  ...
  const navigationItems = [
    ...
    { id: 'platform', label: t('administration'), icon: ShieldCheck },
    { id: 'qa', label: t('qualityAssurance'), icon: ShieldAlert },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row antialiased">
      <a
        href="#main-content"
        className="fixed top-2 start-2 z-[100] -translate-y-20 focus:translate-y-0 bg-indigo-600 text-white px-4 py-2 rounded-lg font-semibold transition-transform"
      >
        {t('skipToContent')}
      </a>
      
      {/* Mobile Top Header Navigation */}
      <header className="md:hidden bg-slate-800 text-white p-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-2">
          <BrandLogoIcon className="w-8 h-8" colorTheme="mono-dark" />
          <span className="font-bold tracking-tight text-sm">Bird Academy</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2.5 min-h-11 min-w-11 inline-flex items-center justify-center text-slate-200 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white rounded-lg"
      ...
      {/* Sidebar Navigation (Desktop version: static & hidden on mobile) */}
      <aside className="hidden md:flex w-64 bg-slate-800 text-slate-300 flex-col justify-between shrink-0 sticky top-0 h-screen z-40 shadow-md border-r border-slate-900">
        <div className="p-5 flex-1 flex flex-col overflow-hidden">
          {/* Header branding */}
          <div className="flex items-center justify-between pb-6 border-b border-slate-700/50 mb-5 gap-2">
            <div className="flex items-center gap-2.5">
              <BrandLogoIcon className="w-10 h-10" colorTheme="mono-dark" />
              <div>
                <h1 className="font-bold tracking-tight text-white leading-tight text-sm">Bird Academy</h1>
                <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block mt-0.5">{t('appTagline')}</span>
              </div>
            </div>
          </div>

          {/* Desktop Language Selector Widget */}
          <div className="mb-5">
            <LanguageSelector variant="dark" />
          </div>
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`
                    w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all text-left cursor-pointer group
                    ${isActive 
                      ? 'bg-amber-500 text-white shadow-xs font-bold' 
                      : 'hover:bg-slate-700/60 hover:text-white text-slate-400'
                    }
                  `}
        ...
        {/* Sidebar Footer */}
        <div className="p-4 border-t border-slate-700 bg-slate-900/40 text-center text-[10px] text-slate-500">
          <span>Bird Academy v1.0 GM</span>
        </div>
      </aside>
        ...
        {mobileMenuOpen && (
          <div className={`fixed inset-0 z-50 md:hidden flex ${isRtl ? 'justify-end' : 'justify-start'}`}>
            ...
            <motion.aside
              ...
              animate="animate"
              exit="exit"
              className={`relative w-72 max-w-[80vw] bg-slate-800 text-slate-300 h-full flex flex-col justify-between shadow-2xl z-10 ${isRtl ? 'border-l' : 'border-r'} border-slate-900`}
              role="dialog"
              aria-modal="true"
              ...
              <div className="p-5 flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between pb-6 border-b border-slate-700/50 mb-5 gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-2.5 min-h-11 min-w-11 inline-flex items-center justify-center hover:bg-slate-750 text-slate-400 hover:text-white rounded-lg transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
      ...
      {/* Main Content Area */}
      <main id="main-content" tabIndex={-1} className="flex-1 overflow-y-auto p-4 md:p-8 max-w-7xl mx-auto w-full min-w-0">
