...
import { useSpeciesProfile } from '../features/species/hooks/useSpeciesProfile';
...
import { 
  AppInput, AppSelect, AppCard, AppAlert, AppButton, AppTable, AppBadge, SpeciesBadge, AppEmptyState, AppModal
} from './design-system';
...
export const LOCAL_I18N: Record<string, Record<string, string>> = {
  fr: {
    title: "Oiseaux",
    subtitle: "Gestion du cheptel, généalogie structurée, historique d'activité, galerie et documents.",
    ...
    addFormTitle: "Fiche d'identification biologique",
  ...
  en: {
    title: "Birds",
    subtitle: "Flock management architecture, structured genealogy, activity history, gallery and documents.",
    ...
    addFormTitle: "Biological Profile Form",
  ...
  ar: {
    ...
    addFormTitle: "بطاقة التعريف البيولوجية",
  ...
  es: {
    title: "Aves",
    subtitle: "Arquitectura de gestión del plantel, genealogía estructurada, historial de actividad, galería y documentos.",
    ...
    addFormTitle: "Ficha de Identificación Biológica",
  ...
  it: {
    title: "Uccelli",
    subtitle: "Gestione del patrimonio, genealogia strutturata, cronologia delle attività, galleria e documenti.",
...
interface CanarisProps {
  ...
  onNavigateToSimulator?: (bird: Canari) => void;
...
export default function Canaris({
  ...
  // Active species reactively scoped from user profile
  const { activeSpecies: activeSpeciesList } = useSpeciesProfile();

  const activeSpeciesCategories = useMemo(() => {
    if (filterSpecies) {
      const sp = getSpeciesById(filterSpecies);
      return sp?.categories || [];
    }
    return activeSpeciesList.flatMap(s => s.categories);
  }, [filterSpecies, activeSpeciesList]);

  // Dynamic values based on selected Species/Category in the Formulaire V2
  const currentFormSpeciesData = useMemo(() => {
    return getSpeciesById(formData.espece || activeSpeciesList[0]?.id || 'canari');
  }, [formData.espece, activeSpeciesList]);
  ...
  const filteredBirds = useMemo(() => {
    ...
    // Strict scoping to user's active species profile when no specific species filter is selected
    if (!filterSpecies) {
      list = list.filter(b => !b.espece || SpeciesProfileService.isSpeciesActive(b.espece));
    }
  ...
  const getCageName = useCallback((cageId?: number | null) => {
    if (cageId === undefined || cageId === null) return '-';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">{localT('labelSpecies')}</label>
                    {setCurrentTab && (
                      <button 
                        type="button" 
                  ...
                  <AppSelect 
                    label=""
                    value={formData.espece || activeSpeciesList[0]?.id || 'canari'}
                    onChange={(e) => handleSpeciesChange(e.target.value)}
                    error={formErrors.espece}
                    options={activeSpeciesList.map(s => ({ value: s.id, label: s.defaultLabel }))}
                  />
                </div>
            ...
            {formTab === 'localisation' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <AppSelect 
                  ...
                  value={formData.cageId || (formData.cage_id !== undefined && formData.cage_id !== null ? String(formData.cage_id) : '')}
                  onChange={(e) => {
                    const val = e.target.value;
                    if (!val) {
                      handleInputChange('cage_id', undefined);
                      handleInputChange('cageId', undefined);
                      handleInputChange('zoneId', undefined);
                      handleInputChange('facilityId', undefined);
                    } else {
                      const numVal = parseInt(val, 10);
                      handleInputChange('cage_id', !isNaN(numVal) ? numVal : undefined);
                      handleInputChange('cageId', val);
                    }
                  }}
                  error={formErrors.cage_id}
                  options={[
                    { value: '', label: 'Aucune affectation' },
                    ...cages.map(c => ({ value: c.id, label: `${c.nom} (max ${c.capacite_max || 6})` }))
                  ]}
                />
  ...
  return (
    <div className="space-y-6 animate-fadeIn" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <div className="flex flex-wrap gap-2.5">
          ...
          <AppButton 
            ...
            onClick={() => {
              ...
              setSelectedBird(null);
              setFormTab('identity');
              const defaultSpecies = activeSpeciesList[0]?.id || 'canari';
              const defaultSpecData = getSpeciesById(defaultSpecies);
              const defaultCategory = defaultSpecData?.categories[0]?.id || 'canari_couleur';
              const defaultBreed = defaultSpecData?.categories[0]?.breeds[0]?.id || 'Classique';

              setFormData({
                nom: '',
                bague: '',
                sexe: 'Mâle',
                espece: defaultSpecies,
      ...
      {/* Main full width table / list area */}
      ...
      {/* Selected bird detail modal */}
      {selectedBird && (
        <BirdDetailModal
          bird={selectedBird}
          onClose={() => setSelectedBird(null)}
          onSelectBird={(b) => setSelectedBird(b)}
          onEditBird={(b) => {
            setFormData({ ...b });
            setFormTab('identity');
            setIsEditing(true);
          ...
          onPairBird={(b) => {
            localStorage.setItem('ba_selected_pair_bird', JSON.stringify(b));
            setSelectedBird(null);
            if (onNavigateToSimulator) {
              onNavigateToSimulator(b);
            } else if (setCurrentTab) {
              setCurrentTab('genetics');
            }
          }}
          onDuplicateBird={(b) => handleDuplicateBird(b)}
          onArchiveBird={(b) => handleArchiveBird(b)}
          onRestoreBird={(b) => handleRestoreBird(b)}
          onDeleteBird={(b) => handleDeleteBird(b)}
