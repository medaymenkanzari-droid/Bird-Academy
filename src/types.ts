...
export interface Canari {
  ...
  cage_id?: number | null; // Foreign Key to Cage
  ...
  archived?: boolean;
  quarantaine?: QuarantaineInfo;
  photos?: string[];
  documents?: {
    id: string;
    nom: string;
    type: 'certificat' | 'analyse' | 'facture' | 'autre';
    date: string;
    description?: string;
  }[];
  elevage?: string;
  ...
  voliere?: string;
  compartiment?: string;
  eleveur_origine?: string;
  parent_pere_nourricier_id?: number | null;
  parent_mere_nourriciere_id?: number | null;
  statut_sante?: string;
  observations?: string;
  acquisition?: boolean;
  facilityId?: string;
...
export interface QuarantaineInfo {
  date_entree: string;
