...
export class HabitatEngine {
  ...
  static calculateStats(
    ...
    if (entityType === 'compartment') {
      ...
      const cage = cages.find(c => String(c.id) === String(entityId)) || { id: entityId, nom: entityId, capacite_max: 10 };
      if (cage) {
        const occ = calculateCageOccupancy(cage as any, birds, compartments);
        capaciteTotale = occ.capacity;
        oiseauxPresents = occ.presentBirds.length;
      }
      ...
      if (zone) {
        ...
        const zoneCageObjects = cages.filter(c => c.zoneId === entityId && !c.isArchived);
        const cageIds = zoneCageObjects.map(c => String(c.id));

        const presentBirdIds = new Set<number>();
        zoneCageObjects.forEach(c => {
          const occ = calculateCageOccupancy(c, birds, compartments);
          occ.presentBirds.forEach(b => presentBirdIds.add(b.id));
        });

        birds.forEach(b => {
          if (!b.archived && b.statut_sante !== 'Décédé' && (b as any).statut !== 'Vendu') {
            if (
              (b.zoneId === entityId && (!b.cageId && !b.cage_id)) ||
        ...
        oiseauxPresents = presentBirdIds.size;
      }
      ...
      if (fac) {
        ...
        const facZoneIds = new Set(facZones.map(z => z.id));
        ...
        const facCages = cages.filter(c => c.zoneId && facZoneIds.has(c.zoneId) && !c.isArchived);
        ...
        birds.forEach(b => {
          if (!b.archived && b.statut_sante !== 'Décédé' && (b as any).statut !== 'Vendu') {
            if (b.facilityId === entityId || (b.zoneId && facZoneIds.has(b.zoneId))) {
              presentBirdIds.add(b.id);
            }
          }
        });
