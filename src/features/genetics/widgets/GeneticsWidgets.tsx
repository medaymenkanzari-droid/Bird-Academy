...
export const GeneticScoreCard: React.FC<GeneticScoreCardProps> = ({
  ...
  const styles = {
    excellent: {
      ...
      labelStr: gt('genetics.scoreExcellent')
    },
    good: {
      ...
      labelStr: gt('genetics.scoreGood')
    },
    medium: {
      ...
      labelStr: gt('genetics.scoreMedium')
    },
    poor: {
      ...
      labelStr: gt('genetics.scorePoor')
    },
    critical: {
      ...
      labelStr: gt('genetics.scoreCritical')
    }
  };

  const current = styles[label] || styles.medium;
  const relLevel = confidence === 'high' ? gt('genetics.reliabilityHigh') : confidence === 'medium' ? gt('genetics.reliabilityMedium') : gt('genetics.reliabilityLow');
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          ...
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400`}>
            {gt('genetics.reliabilityLabel', { level: relLevel })}
          </span>
        </div>
      </div>
      <div className="flex items-center gap-1.5 mb-1.5">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200">{gt('genetics.geneticQuality')}</h3>
...
export const InbreedingGauge: React.FC<InbreedingGaugeProps> = ({ wrightResult }) => {
  ...
  const colorMap = {
    unknown: { stroke: 'stroke-slate-400', text: 'text-slate-500', bg: 'bg-slate-500/10', label: gt('genetics.gauge.unknown') },
    none: { stroke: 'stroke-emerald-500', text: 'text-emerald-500', bg: 'bg-emerald-500/10', label: gt('genetics.gauge.none') },
    low: { stroke: 'stroke-teal-500', text: 'text-teal-500', bg: 'bg-teal-500/10', label: gt('genetics.gauge.low') },
    moderate: { stroke: 'stroke-amber-500', text: 'text-amber-500', bg: 'bg-amber-500/10', label: gt('genetics.gauge.moderate') },
    high: { stroke: 'stroke-orange-500', text: 'text-orange-500', bg: 'bg-orange-500/10', label: gt('genetics.gauge.high') },
    critical: { stroke: 'stroke-rose-500', text: 'text-rose-500', bg: 'bg-rose-500/10', label: gt('genetics.gauge.critical') },
  };
  ...
  const displayPercentage = coefficient === null ? 0 : Math.min(100, (coefficient / 25) * 100);
  ...
  return (
    <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
      <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">{gt('genetics.gauge.title')}</h3>
      ...
      <div className="relative w-32 h-32 mb-3">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-xl font-black text-slate-800 dark:text-slate-100">{coefficient === null ? '—' : `${coefficient}%`}</span>
      ...
      <div className="flex gap-2 items-start bg-slate-50 dark:bg-slate-800/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60 text-start w-full">
        <Info className="w-4 h-4 text-slate-400 mt-0.5 shrink-0" />
        <p className="text-[10px] text-slate-500 leading-relaxed">
          {gt(`genetics.wright.${wrightResult.explanationCode}`, {
            coverage: wrightResult.pedigreeCoverage,
            depth: wrightResult.pedigreeDepth,
            count: wrightResult.commonAncestors.length,
          })}
        </p>
      </div>
      <p className="mt-2 text-[9px] font-semibold text-slate-400">
        {gt('genetics.gauge.coverage', { coverage: wrightResult.pedigreeCoverage })}
      </p>
    </div>
  );
};
...
export const GenealogySummary: React.FC<GenealogySummaryProps> = ({
  ...
  const precisionLabel = completeness >= 80 ? gt('genetics.precisionHigh') : completeness >= 40 ? gt('genetics.precisionModerate') : gt('genetics.precisionInsufficient');
  ...
  return (
    <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
      <div className="flex items-center gap-2 mb-3.5">
        ...
        <div>
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{gt('genetics.treeSummary')}</h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100/60 dark:border-slate-800/60">
          <span className="text-[10px] font-semibold text-slate-400 block mb-1">{gt('genetics.treeDepth')}</span>
          <span className="text-lg font-black text-slate-800 dark:text-slate-100">{depth} <span className="text-xs font-normal text-slate-500">{gt('genetics.generationsUnit')}</span></span>
        </div>
        <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100/60 dark:border-slate-800/60">
          <span className="text-[10px] font-semibold text-slate-400 block mb-1">{gt('genetics.treeCompleteness')}</span>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-[11px] text-slate-500 font-medium">
          <span>{gt('genetics.evaluationPrecision')}</span>
  ...
  return (
    <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{gt('genetics.founderStats')}</h3>
          <p className="text-xs font-bold text-slate-800 dark:text-slate-200">{gt('genetics.originalGeneticPool')}</p>
        </div>
      </div>

      <div className="space-y-3">
        <div className="flex justify-between items-center py-1.5 border-b border-slate-100 dark:border-slate-800/60 text-xs">
          <span className="text-slate-500 font-medium">{gt('genetics.founderCount')}</span>
          <span className="font-bold text-slate-800 dark:text-slate-200">{founderCount}</span>
        </div>
        <div className="flex justify-between items-center py-1.5 border-b border-slate-100 dark:border-slate-800/60 text-xs">
          <span className="text-slate-500 font-medium">{gt('genetics.diversityIndex')}</span>
          <span className="font-bold text-emerald-600 dark:text-emerald-400">{diversityIndex}%</span>
        </div>
        <div className="flex justify-between items-center py-1.5 text-xs">
          <span className="text-slate-500 font-medium">{gt('genetics.renewalIndex')}</span>
      ...
      <div className="mt-4 p-2.5 bg-indigo-50/50 dark:bg-indigo-950/10 border border-indigo-100/40 dark:border-indigo-900/40 rounded-xl flex gap-2">
        ...
        <p className="text-[10px] text-indigo-700 dark:text-indigo-300 leading-relaxed font-semibold">
          {founderCount < 3 ? gt('genetics.founderWarning') : gt('genetics.founderAdequate')}
        </p>
      </div>
    </div>
  );
