...
interface ConfidenceBadgeProps {
  ...
  labels?: Partial<Record<'high' | 'medium' | 'low', string>>;
}

export const ConfidenceBadge: React.FC<ConfidenceBadgeProps> = ({ level, labels }) => {
  const styles = {
    high: {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${current.bg} ${current.text}`}>
      ...
      <span>{labels?.[level] ?? current.label}</span>
    </span>
  );
};
...
export const ProgressScore: React.FC<ProgressScoreProps> = ({ score, label }) => {
  const getGradient = () => {
...
interface ScoreCardProps {
  ...
  confidenceLabels?: Partial<Record<'high' | 'medium' | 'low', string>>;
}
    ...
    <div className={`p-5 bg-white dark:bg-slate-900 rounded-2xl border ${selectedColor.border} shadow-xs hover:shadow-md transition-all`}>
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          ...
          <ConfidenceBadge level={confidence} labels={confidenceLabels} />
        </div>
      </div>
...
export const AlertCard: React.FC<AlertCardProps> = ({
  ...
  actionLabel = 'Action DSS recommandée'
}) => {
        ...
        <div className="flex-1">
          <div className="flex justify-between items-center gap-2 mb-1.5">
            ...
            <span className={`px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider ${
              ...
              {isHigh ? criticalLabel : attentionLabel}
            </span>
          </div>
          ...
          <div className="p-2.5 bg-white/75 dark:bg-slate-900/60 rounded-lg border border-slate-100 dark:border-slate-800/80">
            <div className="text-[9px] font-bold uppercase tracking-wide text-indigo-600 mb-0.5">{actionLabel}</div>
