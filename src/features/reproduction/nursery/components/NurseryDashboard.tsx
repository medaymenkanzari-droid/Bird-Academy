...
export default function NurseryDashboard() {
  ...
  const t = (key: string, defaultVal?: string) => translate(key) || defaultVal || key;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Visual Header */}
      <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/5 p-6 rounded-2xl border border-amber-200/40 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4" dir={isRtl ? 'rtl' : 'ltr'}>
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800">{t('nurseryTitle', 'Gestion de la Nurserie Scientifique')}</h2>
              <p className="text-xs text-slate-500 mt-0.5">{t('nurserySubtitle', "Suivi des portées adoptives, de l'élevage à la main (EAM), des soins intensifs et de la régulation thermique.")}</p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2.5">
          <AppButton size="sm" variant="outline" onClick={() => setIsTransferModalOpen(true)} className="flex items-center gap-1.5 cursor-pointer text-xs">
            ...
            <span>{t('nurseryTransferBtn', 'Transférer un oisillon')}</span>
          </AppButton>
          <AppButton size="sm" variant="success" onClick={() => setIsFeedModalOpen(true)} className="flex items-center gap-1.5 cursor-pointer text-xs">
            ...
            <span>{t('nurseryFeedBtn', 'Nourrir un oisillon')}</span>
          </AppButton>
          <AppButton size="sm" variant="primary" onClick={() => setIsRescueModalOpen(true)} className="flex items-center gap-1.5 cursor-pointer text-xs">
            ...
            <span>{t('nurseryRescueBtn', 'Admettre en sauvetage')}</span>
          </AppButton>
        </div>
      </div>
      ...
      {alerts.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-rose-600">
            ...
            <span className="text-xs font-bold uppercase tracking-wider">{t('nurseryCriticalAlertsTitle', 'Alertes critiques et urgences biologiques')} ({alerts.length})</span>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: t('nurseryKpiBiological', 'Jeunes Biologiques'), value: stats.biologicalCount, icon: Baby, color: "bg-blue-50 text-blue-600" },
          { label: t('nurseryKpiAdopted', 'Adoptés'), value: stats.adoptedCount, icon: Heart, color: "bg-pink-50 text-pink-600" },
          { label: t('nurseryKpiHandFed', 'Élevés à la main'), value: stats.handFedCount, icon: Syringe, color: "bg-amber-50 text-amber-600" },
          { label: t('nurseryKpiOverloadedNests', 'Nids Surchargés'), value: stats.overloadedNestsCount, icon: AlertTriangle, color: stats.overloadedNestsCount > 0 ? "bg-rose-100 text-rose-600" : "bg-slate-50 text-slate-400" },
          { label: t('nurseryKpiActiveFosters', 'Nourriciers Actifs'), value: stats.activeFosterParentsCount, icon: UserCheck, color: "bg-green-50 text-green-600" },
          { label: t('nurseryKpiSurvivalRate', 'Taux de Survie'), value: `${stats.survivalRate}%`, icon: TrendingUp, color: "bg-purple-50 text-purple-600" }
        ].map((kpi, index) => (
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="flex border-b border-slate-100 overflow-x-auto">
          {[
            { id: 'overview', label: t('nurseryTabOverview', 'Vue Générale & Croissance'), icon: Activity },
            { id: 'foster', label: t('nurseryTabFosters', 'Parents Nourriciers (Ado)'), icon: Heart },
            { id: 'handfeeding', label: t('nurseryTabHandfeeding', 'Élevage à la main (EAM)'), icon: Syringe },
            { id: 'rescue', label: t('nurseryTabRescue', "Sauvetages d'Urgence"), icon: LifeBuoy },
            { id: 'protocols', label: t('nurseryTabProtocols', "Protocoles d'Alimentation"), icon: FileSpreadsheet }
          ].map(tab => (
            <button
              key={tab.id}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex justify-between items-center">
                  <div className="min-w-0">
                    <h3 className="text-sm font-bold text-slate-700">{t('nurseryGrowthCurveTitle', 'Courbe de Croissance Biologique')}</h3>
                    <p className="text-[11px] text-slate-500">{t('nurseryGrowthCurveSubtitle', "Comparez le poids mesuré de l'oisillon avec la courbe théorique de référence.")}</p>
                  </div>
                  <div className="w-44">
                    <AppSelect
                      value={chartChickId}
                  ...
                  {growthData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={growthData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <Area name={t('nurseryActualWeight', 'Poids Réel (g)')} type="monotone" dataKey="weight" stroke="#d97706" strokeWidth={2.5} fillOpacity={1} fill="url(#colorActual)" />
                        <Area name={t('nurseryTheoreticalCurve', 'Courbe Théorique (g)')} type="monotone" dataKey="expectedWeight" stroke="#3b82f6" strokeWidth={1.5} strokeDasharray="4 4" fillOpacity={1} fill="url(#colorExpected)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="flex h-full items-center justify-center text-slate-400 text-xs italic">
                      {t('nurseryEmptyGrowth', 'Aucune donnée de croissance disponible.')}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="lg:col-span-1 space-y-4">
                <h3 className="text-sm font-bold text-slate-700">{t('nurseryChicksListTitle', 'Oisillons en Nurserie')} ({activeChicks.length})</h3>
                  ...
                  <div className="w-56">
                    <AppSelect
                      ...
                      options={[
                        ...
                        ...allPairs.filter(p => p.status === 'active' && !fosterParents.some(fp => fp.pairId === p.id)).map(p => ({ value: p.id, label: p.name || `Couple ${p.id}` }))
                      ]}
                    />
                  </div>
                </div>
