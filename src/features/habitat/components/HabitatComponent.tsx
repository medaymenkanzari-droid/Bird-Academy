...
import { 
  ...
  User, Search, ArrowRightLeft, ArrowLeft, AlertTriangle, CheckCircle, Info, RefreshCw as ResetIcon
} from 'lucide-react';
...
import { calculateCageOccupancy } from '../utils/cageOccupancy';
...
export default function HabitatComponent({ canaris: propCanaris, cages: propCages }: HabitatComponentProps = {}) {
  ...
  // Handle popstate & back navigation for hierarchy node selection
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      if (event.state?.habitatDetail) {
        setSelectedType(event.state.habitatDetail.type || 'all');
        setSelectedId(event.state.habitatDetail.id || 'all');
      } else {
        setSelectedType('all');
        setSelectedId('all');
      }
  ...
  const selectNode = (type: typeof selectedType, id: string) => {
    console.log(`[BUG08-RUNTIME-11] Selected hierarchy node: type=${type}, id=${id}`);
    setSelectedType(type);
    setSelectedId(id);
    if (type !== 'all' || id !== 'all') {
      if (window.history.state?.habitatDetail) {
        window.history.replaceState({ habitatDetail: { type, id } }, '');
      } else {
        window.history.pushState({ habitatDetail: { type, id } }, '');
      }
    }
  };
  ...
  const [selectedBirdForModal, setSelectedBirdForModal] = useState<Canari | null>(null);
  ...
  useEffect(() => {
    ...
    import('@capacitor/app').then(({ App }) => {
      App.addListener('backButton', () => {
        // 1. If a modal is open, close the modal first without deselecting node
        if (qrModalOpen || addModalType || moveModalOpen || quarantineModalOpen) {
        ...
        // 2. If a cage or node is selected, return to main cage list
        if (selectedType !== 'all' || selectedId !== 'all') {
          resetSelection();
          return;
        }
  ...
  }, [selectedType, selectedId, qrModalOpen, addModalType, moveModalOpen, quarantineModalOpen]);
  ...
  // Load all entities
  useEffect(() => {
    const loadedFacilities = HabitatRepository.getAll<Facility>('facility');
    const loadedCages = HabitatRepository.getAll<HabitatCage>('cage');
    const loadedBirds = BirdService.getBirds(true);

    const totalBirds = loadedBirds.length;
    const activeBirds = loadedBirds.filter(b => !b.archived && b.statut_sante !== 'Décédé' && (b as any).statut !== 'Vendu').length;

    console.log(`[BUG08-RUNTIME-06] Habitat cages loaded: count=${loadedCages.length}`);
    console.log(`[BUG08-RUNTIME-07] Habitat birds loaded: count=${loadedBirds.length}, active=${activeBirds}`);
    ...
    setMovements(HabitatRepository.getAll<DeplacementRecord>('deplacementRecord').slice().reverse());
    setQuarantines(HabitatRepository.getAll<QuarantineRecord>('quarantineRecord').slice().reverse());
    setBirds(loadedBirds);

    console.log('[BUG08-RUNTIME-15] Cage reload completed post-hydration');
  }, [tick, propCanaris, propCages]);
  ...
  const getBirdCountForNode = (type: string, id: string): number => {
    if (type === 'cage') {
      return calculateCageOccupancy({ id }, birds, compartments).presentBirds.length;
    }
    if (type === 'facility' || type === 'zone' || type === 'aviary' || type === 'quarantineArea') {
      return HabitatEngine.calculateStats(type as any, id, birds).oiseauxPresents;
    }
    if (type === 'compartment') {
      return (birds || []).filter(b => !b.archived && b.statut_sante !== 'Décédé' && (b as any).statut !== 'Vendu' && String(b.compartmentId) === String(id)).length;
    }
    return 0;
  };

  // Stable Global Complex Stats for Top Banner
  const getGlobalStats = () => {
    let capacity = 0;
    let occupancy = 0;
    const allCages = (cages || []).filter(c => !c.isArchived);
    const activeBirds = (birds || []).filter(b => !b.archived && b.statut_sante !== 'Décédé' && (b as any).statut !== 'Vendu');

    if (facilities.length > 0) {
      facilities.forEach(f => {
        const stats = HabitatEngine.calculateStats('facility', f.id, activeBirds);
        capacity += stats.capaciteTotale;
      });
    } else {
      capacity = allCages.reduce((sum, c) => sum + Math.max(0, c.capacite_max || 0), 0);
    }

    const presentBirdIds = new Set<number>();
    allCages.forEach(c => {
      const occ = calculateCageOccupancy(c, activeBirds, compartments);
      occ.presentBirds.forEach(b => presentBirdIds.add(b.id));
    });
    activeBirds.forEach(b => {
      if (b.cageId || b.cage_id || b.zoneId || b.facilityId) {
        presentBirdIds.add(b.id);
      }
    });

    occupancy = presentBirdIds.size;
    ...
    console.log(`[CAGE-DATA-07] totalCapacity = ${capacity}, totalPresentBirds = ${occupancy}, globalOccupancyPercentage = ${rate}%`);
    console.log(`[CAGE-DATA-08] displayedBirdCount = ${occupancy}, displayedOccupancy = ${rate}%`);
  ...
  // Helper: Get birds belonging to a zone
  const getZoneBirds = (zoneId: string) => {
    const zoneCageIds = new Set((cages || []).filter(c => c.zoneId === zoneId).map(c => String(c.id)));
    return (birds || []).filter(b => {
      if (!b || b.archived || b.statut_sante === 'Décédé' || (b as any).statut !== 'Vendu') return false;
      const bCageId = b.cageId ? String(b.cageId) : (b.cage_id ? String(b.cage_id) : undefined);
      return b.zoneId === zoneId || (bCageId && zoneCageIds.has(bCageId));
    });
  };

  // Helper: Get birds present in a cage
  const getCageBirds = (cageId: string) => {
    const cage = (cages || []).find(c => String(c.id) === String(cageId));
    if (!cage) return [];
    return calculateCageOccupancy(cage, birds || [], compartments || []).presentBirds;
  };
  ...
  const openCreateCageModal = (preferredZoneId?: string) => {
    console.log('[BUG08-P3-CREATE-01] open create cage');
    ...
    setCageForm({ nom: '', description: '', zoneId: targetZoneId, aviaryId: '', capacite: 6 });
    console.log('[BUG08-P3-CREATE-02] cage form initialized');
    console.log('[BUG08-P3-CREATE-03] modal data loaded');
    setAddModalType('cage');
  };

  const handleCreateCage = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('[BUG08-P3-CREATE-04] cage repository create');
    if (!cageForm.nom || !cageForm.zoneId) {
      alert("Veuillez renseigner le nom de la cage et la zone parente.");
      return;
    }
    const currentZones = HabitatRepository.getAll<Zone>('zone') || [];
    const targetZone = currentZones.find(z => z && z.id === cageForm.zoneId);
    const created = HabitatRepository.create<HabitatCage>('cage', {
      nom: cageForm.nom,
      description: cageForm.description,
      zoneId: cageForm.zoneId,
      aviaryId: cageForm.aviaryId || undefined,
      capacite_max: cageForm.capacite || 6,
      statut: 'Actif',
      customFields: targetZone ? { facilityId: targetZone.facilityId } : {}
    });
    console.log(`[BUG08-RUNTIME-14] Cage created: id=${created.id}, nom=${cageForm.nom}, zoneId=${cageForm.zoneId}`);
    setCageForm({ nom: '', description: '', zoneId: '', aviaryId: '', capacite: 6 });
    console.log('[BUG08-P3-CREATE-05] cage state updated');
    setAddModalType(null);
    console.log('[BUG08-P3-CREATE-06] modal closed');
  ...
  // Filter List based on strict hierarchy, search, status
  const getFilteredStructures = () => {
    ...
    if (selectedType === 'all') {
      ...
      items = [
        ...
        ...(quarantineAreas || []).filter(q => (q as any).zoneId === selectedId).map(q => ({ ...q, type: 'quarantineArea' }))
      ];
    ...
    console.log(`[BUG08-RUNTIME-12] Filtered structures: count=${items.length}, selectedType=${selectedType}, selectedId=${selectedId}`);
    return items;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSubTab === 'structures' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {facilities.map(facility => {
                  ...
                  return (
                    <div key={facility.id} className="space-y-1">
                      <div 
                        onClick={() => selectNode('facility', facility.id)}
                      ...
                      >
                        ...
                        <span className="text-[10px] bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded-full">
                          {getBirdCountForNode('facility', facility.id)}/{(facility as any).capacite || 100}
                        </span>
                      </div>
                          ...
                          {childZones.map(zone => {
                            ...
                            return (
                              <div key={zone.id} className="space-y-1">
                                <div 
                                  onClick={() => selectNode('zone', zone.id)}
                                  ...
                                  <div className="pl-4 border-l border-slate-100 ml-3 space-y-1">
                                    ...
                                    {childAviaries.map(aviary => {
                                      ...
                                      return (
                                        <div 
                                          key={aviary.id}
                                          onClick={() => selectNode('aviary', aviary.id)}
                                        ...
                                        >
                                          ...
                                          <span className="text-[9px] bg-slate-100 text-slate-500 px-1 rounded-full">{getBirdCountForNode('aviary', aviary.id)}/{(aviary as any).capacite || 20}</span>
                                        </div>
                                      );
                                    })}
                                    ...
                                    {childCages.map(cage => {
                                      ...
                                      return (
                                        <div 
                                          key={cage.id}
                                          onClick={() => selectNode('cage', cage.id)}
          ...
          <div className="lg:col-span-8 bg-white p-5 rounded-xl border border-slate-100 shadow-xs space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-100 pb-4">
              <div className="space-y-0.5 flex items-center gap-3">
                {selectedType !== 'all' && (
                  <button
                    onClick={resetSelection}
                    className="p-1.5 text-slate-600 hover:text-amber-600 bg-slate-100 hover:bg-amber-50 rounded-lg transition-colors cursor-pointer inline-flex items-center gap-1 text-xs font-semibold"
                    title="Retour à la liste des cages"
                  >
                ...
                <AppButton 
                  onClick={() => {
                    if (selectedType === 'facility') {
                      setZoneForm(prev => ({ ...prev, facilityId: selectedId }));
                      setAddModalType('zone');
                    } else if (selectedType === 'zone') {
                      console.log(`[BUG08-RUNTIME-13] Create cage modal opened: prefilledZoneId=${selectedId}`);
                      openCreateCageModal(selectedId);
                    } else if (selectedType === 'cage') {
                      setCompartmentForm(prev => ({ ...prev, cageId: selectedId }));
                      setAddModalType('compartment');
                    } else {
                      openCreateCageModal();
                    }
                  }}
            ...
            {/* Structured Grid Table / Empty State Logic */}
            {selectedType === 'cage' ? (
              filteredStructures.length > 0 && (
                <div className="space-y-2 mb-4">
                  <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                    Compartiments ({filteredStructures.length})
                  </h4>
                  <HorizontalScrollContainer innerClassName="min-w-[500px]">
                    <AppTable
                      data={filteredStructures}
                      keyExtractor={(item: any) => item.id}
                      onRowClick={(item: any) => selectNode(item.type, item.id)}
                      columns={[
                        {
                          key: 'type',
                          header: 'Type',
                        ...
                        },
                        {
                          ...
                          render: (item: any) => (
                            <div className="flex justify-end gap-1.5">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); handleDeleteEntity(item.type, item.id); }}
                                title="Supprimer la structure"
                                className="p-1.5 text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer inline-flex items-center"
                              >
              ...
              ((selectedType === 'zone' && getZoneBirds(selectedId).length > 0) ||
               (selectedType === 'facility' && HabitatEngine.calculateStats('facility', selectedId, birds).oiseauxPresents > 0)) ? null : (
                <AppEmptyState 
              ...
              <div className="mt-6 border-t border-slate-100 pt-4 space-y-3">
                <div className="flex justify-between items-center flex-wrap gap-2">
                  <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
                    ...
                    Oiseaux de la zone ({getZoneBirds(selectedId).length})
                  </h4>
                  <span className="text-[10px] text-slate-400 font-medium md:hidden">
                    ← Glisser pour tout voir →
                  </span>
                </div>
                {getZoneBirds(selectedId).length === 0 ? (
                  <AppEmptyState 
                    title="Aucun oiseau dans cette zone"
                    description="Aucun oiseau n'est actuellement affecté aux cages de cette zone."
                  />
                ) : (
                  <HorizontalScrollContainer innerClassName="min-w-[320px] md:min-w-[650px]">
                    <AppTable
                      data={getZoneBirds(selectedId)}
                      keyExtractor={(b: any) => String(b.id)}
                      onRowClick={(b: any) => setSelectedBirdForModal(b)}
                      hoverable={true}
                      columns={[
                        ...
                        { 
                          ...
                          render: (b: any) => (
                            <button
                              onClick={(e) => { e.stopPropagation(); setSelectedBirdForModal(b); }}
                              className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-800 rounded text-xs font-bold transition-colors cursor-pointer"
                            >
                              Fiche
                            </button>
                          ) 
            ...
            {selectedType === 'cage' && (
              <div className="mt-6 border-t border-slate-100 pt-4 space-y-3">
                <div className="flex justify-between items-center flex-wrap gap-2">
                  <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
                    <User className="w-4 h-4 text-amber-500" />
                    Oiseaux présents dans la cage ({getCageBirds(selectedId).length})
                  </h4>
                  <span className="text-[10px] text-slate-400 font-medium md:hidden">
                    ← Glisser pour tout voir →
                  </span>
                </div>
                {getCageBirds(selectedId).length === 0 ? (
                  <AppEmptyState 
                    title="Aucun oiseau dans cette cage"
                    description="Cette cage est actuellement vide."
                  />
                ) : (
                  <HorizontalScrollContainer innerClassName="min-w-[320px] md:min-w-[650px]">
                    <AppTable
                      data={getCageBirds(selectedId)}
                      keyExtractor={(b: any) => String(b.id)}
                      onRowClick={(b: any) => setSelectedBirdForModal(b)}
                      hoverable={true}
                      columns={[
      ...
      >
        {birds.length === 0 ? (
          <AppEmptyState
            title="Aucun oiseau enregistré"
            description="Veuillez d'abord ajouter des oiseaux dans votre volière avant d'effectuer un déplacement."
          />
        ) : (
          <form onSubmit={handleMoveSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Oiseau à transférer</label>
              <AppSelect
              ...
              >
                ...
                {birds.map(b => (
                  <option key={b.id} value={b.id}>
                    {b.nom || 'Sans nom'} ({b.bague || 'Sans bague'}) - {typeof b.statut_sante === 'string' ? b.statut_sante : 'Actif'}
                  </option>
                ))}
              </AppSelect>
            </div>
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Type de Destination</label>
              <AppSelect
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Destination Exacte</label>
              <AppSelect
              ...
              >
                ...
                {moveForm.destType === 'cage' && (cages || []).map(c => <option key={c.id} value={c.id}>{c.nom} (Cap. {(c as any).capacite_max || (c as any).capacite || 6})</option>)}
                {moveForm.destType === 'aviary' && (aviaries || []).map(a => <option key={a.id} value={a.id}>{a.nom} (Cap. {(a as any).capacite || 20})</option>)}
                {moveForm.destType === 'compartment' && (compartments || []).map(comp => <option key={comp.id} value={comp.id}>{comp.nom} (Cap. {(comp as any).capacite || 2})</option>)}
                {moveForm.destType === 'zone' && (zones || []).map(z => <option key={z.id} value={z.id}>{z.nom}</option>)}
                {moveForm.destType === 'quarantineArea' && (quarantineAreas || []).map(q => <option key={q.id} value={q.id}>{q.nom} (Cap. {(q as any).capacite || 10})</option>)}
              </AppSelect>
            </div>

            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Motif du transfert</label>
              <AppSelect
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Commentaires</label>
              <AppInput 
      ...
      >
        {birds.length === 0 ? (
          <AppEmptyState
            title="Aucun oiseau enregistré"
            description="Aucun oiseau n'est disponible pour la mise en quarantaine."
          />
        ) : (
          <form onSubmit={handleQuarantineSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Oiseau à isoler</label>
              <AppSelect
              ...
              >
                ...
                {birds.filter(b => b && String(b.statut_sante || '').toLowerCase() !== 'quarantaine').map(b => (
                  <option key={b.id} value={b.id}>
                    {b.nom || 'Sans nom'} ({b.bague || 'Sans bague'}) - {typeof b.statut_sante === 'string' ? b.statut_sante : 'Actif'}
                  </option>
                ))}
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Zone d'isolement</label>
              <AppSelect
              ...
              >
                ...
                {(quarantineAreas || []).map(q => {
                  const cap = (q as any).capacite || 10;
                  const count = getBirdCountForNode('quarantineArea', q.id);
                  const remaining = Math.max(0, cap - count);
                  return (
                    <option key={q.id} value={q.id}>{q.nom} (Places disponibles: {remaining})</option>
                  );
                })}
              </AppSelect>
              {quarantineAreas.length === 0 && (
                <span className="text-[10px] text-amber-600 font-semibold mt-1 block">
                  ⚠️ Aucune zone de quarantaine configurée. Créez d'abord une zone de quarantaine via « Nouvelle Installation ».
                </span>
              )}
            </div>

            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Durée d'isolement prévue (Jours)</label>
              <AppInput 
                type="number"
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Raison de l'isolement</label>
              <AppSelect
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Traitements prescrits</label>
              <AppInput 
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-1">Observations initiales</label>
              <AppInput 
      ...
      {/* SHARED BIRD DETAIL MODAL */}
      {selectedBirdForModal && (
        <BirdDetailModal
          bird={selectedBirdForModal}
          onClose={() => setSelectedBirdForModal(null)}
          onSelectBird={(b) => setSelectedBirdForModal(b)}
          allBirds={birds}
        />
      )}
    </div>
