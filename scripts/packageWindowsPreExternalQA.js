 ...
 * 
 * BIRD ACADEMY ENTERPRISE — PRE-EXTERNAL-QA-01 WINDOWS RELEASE PACKAGING PIPELINE
 * Compiles, verifies, and packages both User (Avian ERP) and Admin (Admin Center)
 * Windows binaries for external QA testing.
 * 
 * Target Directory  : Release/Windows-PreExternalQA/
...
const RELEASE_DIR = path.join(ROOT_DIR, 'Release', 'Windows-PreExternalQA');
...
console.log('==================================================================');
console.log(' BIRD ACADEMY — PRE-EXTERNAL-QA-01 DUAL WINDOWS PACKAGING         ');
console.log(' Target Directory  : Release/Windows-PreExternalQA/               ');
...
// ----------------------------------------------------------------------
// 5. DEPLOYMENT TO Release/Windows-PreExternalQA/ & SHA-256 GENERATION
// ----------------------------------------------------------------------
console.log('\nDeploying binaries to Release/Windows-PreExternalQA/ & generating SHA-256 sums...');
...
const finalAdminSetup = path.join(RELEASE_DIR, 'Bird-Academy-Admin-Setup.exe');
...
if (fs.existsSync(USER_BUILD_DIR)) {
  ...
  for (const file of userFiles) {
    ...
    if (file.endsWith('.exe') && fs.statSync(fullPath).isFile()) {
      ...
      } else if (!fs.existsSync(finalUserPortable) || lower.includes('user')) {
...
if (fs.existsSync(finalUserSetup)) {
  ...
  shaLines.push(`${hash}  Bird-Academy-Avian-ERP-Setup.exe (${size} MB)`);
...
if (fs.existsSync(finalUserPortable)) {
  ...
  shaLines.push(`${hash}  Bird-Academy-User.exe (${size} MB)`);
...
if (fs.existsSync(finalAdminSetup)) {
  ...
  shaLines.push(`${hash}  Bird-Academy-Admin-Setup.exe (${size} MB)`);
...
if (fs.existsSync(finalAdminPortable)) {
  ...
  shaLines.push(`${hash}  Bird-Academy-Admin.exe (${size} MB)`);
  console.log(`  ✅ Admin Portable : ${finalAdminPortable} (${size} MB) -> SHA256: ${hash}`);
}

const shaFilePath = path.join(RELEASE_DIR, 'SHA256SUMS.txt');
fs.writeFileSync(shaFilePath, shaLines.join('\n') + '\n', 'utf8');

console.log('\n==================================================================');
console.log(' PRE-EXTERNAL-QA-01 WINDOWS RELEASE COMPLETED SUCCESSFULLY!       ');
